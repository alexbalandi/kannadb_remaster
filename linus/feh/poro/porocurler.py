...
from .poroclasses import Hero, Refine, Skill, Weapon
...
def CurlAll():
        ...
        if span.parent.name == "h3":
            ...
            while currentRow < maxRow:
                ...
                for i in range(numRefines):
                    ...
                    statRow = [node for node in currentRefineRow.contents if "rowspan" not in node.attrs][1:]
    ...
    def parseHero(soup, heroName, heroMod, heroURL):
        ...
        def pss(x):
            return list(map(tryStrToInt, x.split("/")))

        def rss(r):
            return list(map(tryStrToInt, r.split("~")))
